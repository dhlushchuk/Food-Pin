...
@Model class Restaurant {
    ...
    @Transient var rating: Rating? {
        get {
            guard let ratingText else { return nil }
            return Rating(rawValue: ratingText)
        }
        set {
            ratingText = newValue?.rawValue
        }
    }
    
    @Attribute(originalName: "rating") var ratingText: Rating.RawValue?
    
    init(name: String, type: String, location: String, phone: String, description: String, image: UIImage? = nil, isFavorite: Bool = false, rating: Rating? = nil) {
        self.name = name
        self.type = type
